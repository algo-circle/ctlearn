...
def dyn_rnn_model(features, params, training):
    """
    CNN-RNN model which uses a FCN head of fixed length to process arbitrary input
    """
    ...
    if not num_telescope_types == 1:
        raise ValueError('Must use a single telescope type for dynamic RNN. Number used: {}'.format(num_telescope_types))
    ...
    with tf.variable_scope("NetworkHead"):
        ...
        # outputs = shape(num_tel, batch_size, output_size)
        ...
        print("output_rnn.shape=" + str(output_rnn.shape))
        output_dropout = tf.layers.dropout(output_rnn, rate=dropout_rate,
